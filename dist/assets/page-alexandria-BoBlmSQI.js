import{u as Ba,j as e,L as Ea,r as x,f as Oa,d as da}from"./vendor-react-CNa2jnXH.js";import{s as w,c as G,C as h,B as C,b as f,A as q,f as Pe,a as j,d as L,e as B,l as J,D as I,k as Ra,g as D,E as Me,m as Fe}from"./page-dashboard-Dk_sLb2Q.js";import{T as ma,f as ua,g as F,h as K,L as Z,I as H,S as $a,a as za,b as qa,c as Ma,d as le,i as Fa,s as Ka,j as Ga,P as pa,k as Ua,l as Ha,O as Wa}from"./page-agents-DS8dF5pd.js";import{t as Ae,H as Ke,e as se,Z as W,a2 as Ie,a0 as Va,_ as Q,R as ge,d as he,au as me,O as X,S as Se,A as fe,F as oe,g as je,ab as be,G as xa,X as Za,a1 as ga,$ as Y,w as ha,av as Ja,s as Qa,I as Xa,Q as ue,aw as Ya,ax as en,ay as an,az as De,k as fa,aA as Ge,a5 as Ue,c as He,Y as re,V as ja,aB as nn,ah as sn,aC as on,z as tn,D as rn,b as ln,K as cn,T as dn,aD as mn,aE as ce,l as ba,W as We,C as Te,aF as ve,aG as un,aH as va,L as pn,f as xn,a9 as gn}from"./vendor-icons-DlnuxFlV.js";import{A as Ve,m as Ze}from"./vendor-motion-CUNXwmoF.js";import{D as hn,a as fn,b as jn,c as bn,d as vn}from"./page-clients-j8VXGiIa.js";import{a as Cn,al as Ca}from"./vendor-visuals-DGpBlf5v.js";function Le(a,n,o){const s=new Blob([a],{type:o}),r=URL.createObjectURL(s),t=document.createElement("a");t.href=r,t.download=n,document.body.appendChild(t),t.click(),t.remove(),URL.revokeObjectURL(r)}const _n=new Set(["para","com","uma","que","dos","das","como","mais","isso","essa","esse","pela","pelo","the","and","with","from","this","that","your","about","into"]),kn=["senha","password","token:","access token","bearer ","api_key","secret","service_role","cpf","cartao","cartão","dados banc"];async function yn(a){if(globalThis.crypto?.subtle){const o=new TextEncoder().encode(a),s=await globalThis.crypto.subtle.digest("SHA-256",o);return Array.from(new Uint8Array(s)).map(r=>r.toString(16).padStart(2,"0")).join("")}let n=0;for(let o=0;o<a.length;o+=1)n=(n<<5)-n+a.charCodeAt(o),n|=0;return`fallback-${Math.abs(n)}`}async function _a(a){const n=a.content.replace(/\r\n?/g,`
`),o=[];let s=Nn(n,o);s!==a.content&&o.push("Conteúdo normalizado antes de salvar."),s=s.normalize("NFC");const r=Tn(a.name,s);return{...a,content:s,originalContent:a.content,contentHash:await yn(s),sanitizationWarnings:R(o),privacyZone:r.privacyZone,privacyReason:r.reason}}function Nn(a,n){let o="";for(const s of a){const r=s.charCodeAt(0);if(r<32&&r!==9&&r!==10&&r!==13||r===127){n.push(`Caractere de controle removido: U+${r.toString(16).padStart(4,"0")}`);continue}o+=s}return o}async function ft(a){const n=await Promise.all(a.map(_a)),o=n.filter(k=>k.privacyZone!=="red"),s=n.filter(k=>k.privacyZone==="red"),r=o.map(k=>Be(k.name,k.content)),t=await wn(o),c=await Pn(r),l=An(o,t),i=In(r,c,o,l),m=Ln(r,c),v=R(r.flatMap(k=>k.gaps)).slice(0,12),b=ya(r.length?r:[{recommendedOutput:"document"}]),g=s.length||m.some(k=>k.severity==="high")||i.length?"review":"draft",_={generatedAt:new Date().toISOString(),files:n,allowedFiles:o,blockedFiles:s,analyses:r,exactDuplicates:l,nearDuplicates:i,conflicts:m,gaps:v,relatedArtifacts:c,recommendedStatus:g,recommendedArtifactType:b};return{..._,reportMarkdown:Bn(_)}}function Be(a,n){const o=$n(n),s=we(`${a} ${n}`).slice(0,12),r=On(a,n),t=zn(n),c=Qe(n,["conflito","contradi","diverg","versus"," vs ","não usar","evitar","problema"]),l=Qe(n,["risco","cuidado","atenção","falha","erro","bloqueio"]),i=qn(n,r),m=Array.from(new Set([r,...s.slice(0,6)])).slice(0,8);return{name:a,detectedType:r,title:o[0]||Hn(a),headings:o,tags:m,keywords:s,keyIdeas:t,risks:l,gaps:i,conflicts:c,recommendedOutput:r==="document"?Rn(n):r}}async function Ee(a){const o=(await Promise.all(a.map(_a))).filter(l=>l.privacyZone!=="red"),s=new Map;for(const l of o){const i=Be(l.name,l.content);s.has(l.contentHash)||s.set(l.contentHash,{title:i.title,file_name:l.name,source_type:l.name.toLowerCase().match(/\.(md|markdown)$/)?"markdown":"text",origin:l.origin||"upload",author:l.author||Un(l.name,l.content),content:l.content,content_hash:l.contentHash,detected_type:i.detectedType,tags:i.tags,metadata:{headings:i.headings,keywords:i.keywords,keyIdeas:i.keyIdeas,risks:i.risks,gaps:i.gaps,conflicts:i.conflicts,recommendedOutput:i.recommendedOutput,originalName:l.name,privacyZone:l.privacyZone,privacyReason:l.privacyReason,sanitizationWarnings:l.sanitizationWarnings}})}const r=Array.from(s.values());if(!r.length)return[];const{data:t,error:c}=await w.from("hermione_sources").upsert(r,{onConflict:"content_hash"}).select("*");if(c)throw new Error(`Não consegui salvar as fontes da Hermione: ${c.message}`);return t||[]}async function jt(a){if(!a.allowedFiles.length)throw new Error("Nenhum arquivo permitido para assimilar na Alexandria.");const n=a.allowedFiles.filter(t=>!a.exactDuplicates.some(c=>c.name===t.name)).map(({name:t,content:c,origin:l,author:i})=>({name:t,content:c,origin:l,author:i})),o=n.length?n:a.allowedFiles.map(({name:t,content:c,origin:l,author:i})=>({name:t,content:c,origin:l,author:i})),s=await Ee(o),r=await Oe(o,{sourceIds:s.map(t=>t.id),type:a.recommendedArtifactType,status:a.recommendedStatus,changeNote:"Assimilação após simulação da Hermione",metadata:{assimilationPreview:{generatedAt:a.generatedAt,exactDuplicates:a.exactDuplicates,nearDuplicates:a.nearDuplicates,conflicts:a.conflicts,blockedFiles:a.blockedFiles.map(t=>({name:t.name,reason:t.privacyReason})),relatedArtifacts:a.relatedArtifacts.map(t=>({id:t.id,title:t.title,status:t.status,artifact_type:t.artifact_type}))},assimilationReport:a.reportMarkdown}});return await Dn({query:`Relatório de assimilação: ${a.allowedFiles.map(t=>t.name).join(", ")}`,response:a.reportMarkdown,sourceIds:s.map(t=>t.id),artifactIds:[r.id],metadata:{exactDuplicates:a.exactDuplicates.length,nearDuplicates:a.nearDuplicates.length,conflicts:a.conflicts.length,blockedFiles:a.blockedFiles.length}}),{sources:s,artifact:r}}function bt(a,n="markdown"){const o=n==="json"?JSON.stringify(a,null,2):a.reportMarkdown;Le(o,`relatorio-assimilacao-hermione.${n==="json"?"json":"md"}`,n==="json"?"application/json":"text/markdown")}function Sn(a){const n=a.map(m=>Be(m.name,m.content)),o=ya(n),s=Mn(n,o),r=R(n.flatMap(m=>m.conflicts)).slice(0,8),t=R(n.flatMap(m=>m.gaps)).slice(0,8),c=R(n.flatMap(m=>m.tags)).slice(0,12),l=Fn(n,o,a.length),i=Kn({title:s,artifactType:o,summary:l,tags:c,analysis:n,conflicts:r,gaps:t});return{title:s,artifactType:o,summary:l,tags:c,analysis:n,content:i,conflicts:r,gaps:t}}async function Oe(a,n={}){const o=Sn(a),s=n.type||o.artifactType,r={title:o.title,artifact_type:s,status:n.status||"draft",scope:n.scope||"totum",content:s===o.artifactType?o.content:Gn(o,s),summary:o.summary,tags:o.tags,metadata:{sourceCount:a.length,conflicts:o.conflicts,gaps:o.gaps,analyses:o.analysis,...n.metadata||{}},version:1},{data:t,error:c}=await w.from("hermione_artifacts").insert(r).select("*").single();if(c)throw new Error(`Não consegui criar o artefato da Hermione: ${c.message}`);const{error:l}=await w.from("hermione_artifact_versions").insert({artifact_id:t.id,version:1,content:r.content,change_note:n.changeNote||"Criação a partir do chat consultivo da Hermione",metadata:r.metadata});if(l)throw new Error(`Artefato criado, mas falhei ao registrar a versão: ${l.message}`);if(n.sourceIds?.length){const{error:i}=await w.from("hermione_artifact_sources").insert(n.sourceIds.map(m=>({artifact_id:t.id,source_id:m,contribution_type:"source"})));if(i)throw new Error(`Artefato criado, mas falhei ao vincular fontes: ${i.message}`)}return t}async function Je(a="",n=12){let o=w.from("hermione_artifacts").select("*").order("updated_at",{ascending:!1}).limit(n);if(a.trim()){const t=a.trim().replace(/[,%]/g,"");o=o.or(`title.ilike.%${t}%,summary.ilike.%${t}%,content.ilike.%${t}%`)}const{data:s,error:r}=await o;return r?(console.warn("Hermione artifacts unavailable:",r.message),[]):s||[]}async function Dn(a){const{error:n}=await w.from("hermione_consultations").insert({query:a.query,response:a.response||null,source_ids:a.sourceIds||[],artifact_ids:a.artifactIds||[],metadata:a.metadata||{}});if(n)throw new Error(`Não consegui registrar a consulta da Hermione: ${n.message}`)}function vt(a,n="markdown"){const o=n==="json"?JSON.stringify(a,null,2):a.content,s=n==="json"?"application/json":"text/markdown",r=n==="json"?"json":"md";Le(o,`${Wn(a.title)}.${r}`,s)}function Tn(a,n){const o=`${a}
${n}`.toLowerCase(),s=kn.find(r=>o.includes(r));return s?{privacyZone:"red",privacyReason:`Sinal sensível detectado: ${s}.`}:/pessoal|rotina|prefer[eê]ncia|logseq|journal|di[aá]rio/i.test(o)?{privacyZone:"yellow",privacyReason:"Contexto pessoal/operacional permitido somente com revisão."}:{privacyZone:"green",privacyReason:"Nenhum sinal sensível conhecido detectado."}}async function wn(a){const n=R(a.map(r=>r.contentHash));if(!n.length)return[];const{data:o,error:s}=await w.from("hermione_sources").select("*").in("content_hash",n);return s?(console.warn("Hermione sources unavailable for preview:",s.message),[]):o||[]}async function Pn(a){const n=R(a.flatMap(r=>[r.title,...r.tags])).slice(0,6);if(!n.length)return Je("",8);const o=await Promise.all(n.map(r=>Je(r,4))),s=new Map;return o.flat().forEach(r=>s.set(r.id,r)),Array.from(s.values()).slice(0,10)}function An(a,n){const o=[],s=new Map,r=new Map(n.map(t=>[t.content_hash,t]));return a.forEach(t=>{const c=r.get(t.contentHash);if(c){o.push({name:t.name,match:c.file_name,severity:"exact",reason:"Mesmo hash de uma fonte já catalogada na Alexandria."});return}const l=s.get(t.contentHash);if(l){o.push({name:t.name,match:l,severity:"exact",reason:"Mesmo hash de outro arquivo selecionado nesta assimilação."});return}s.set(t.contentHash,t.name)}),o}function In(a,n,o,s){const r=new Set(s.map(c=>c.name)),t=[];return a.forEach((c,l)=>{const i=o[l];if(!i||r.has(i.name))return;const m=n.find(v=>c.tags.filter(g=>v.tags?.includes(g)).length>=2||ka(c.title,v.title)>=.65);m&&t.push({name:i.name,match:m.title,severity:"near",reason:`Tema parecido com artefato existente (${m.status}/${m.artifact_type}).`})}),t}function Ln(a,n){const o=a.filter(t=>t.conflicts.length>0).map(t=>({topic:t.title,severity:"high",sources:[t.name],recommendation:"Manter em revisão humana antes de aprovar ou substituir conhecimento existente."})),s=n.filter(t=>t.status==="approved"),r=a.filter(t=>s.some(c=>ka(t.title,c.title)>=.65)).map(t=>({topic:`Possível atualização de conhecimento aprovado: ${t.title}`,severity:"medium",sources:[t.name,...s.map(c=>c.title).slice(0,2)],recommendation:"Não sobrescrever automaticamente. Gerar nova versão em review e pedir aprovação."}));return En([...o,...r],t=>`${t.topic}-${t.sources.join(",")}`).slice(0,12)}function Bn(a){const n=a.allowedFiles.map(i=>`- **${i.name}**: ${i.privacyZone}; hash \`${i.contentHash.slice(0,12)}\`; ${i.privacyReason}`).join(`
...
`}function ka(a,n){const o=new Set(we(a).slice(0,8)),s=new Set(we(n).slice(0,8));return!o.size||!s.size?0:Array.from(o).filter(t=>s.has(t)).length/Math.max(o.size,s.size)}function En(a,n){const o=new Set;return a.filter(s=>{const r=n(s);return o.has(r)?!1:(o.add(r),!0)})}function On(a,n){const o=`${a} ${n}`.toLowerCase();return o.includes("skill.md")||o.includes("quando usar")||o.includes("entrada")&&o.includes("saída")?"skill":o.includes("pop-")||o.includes("procedimento operacional")||o.includes("sla")?"pop":o.includes("system prompt")||o.includes("prompt")||o.includes("instruções do agente")?"prompt":o.includes("decisão")||o.includes("adr")||o.includes("decision record")?"decision":o.includes("contexto")||o.includes("context pack")||o.includes("memória")?"context_pack":o.includes("resumo")||o.includes("summary")?"summary":"document"}function Rn(a){const n=a.toLowerCase();return n.includes("passo")||n.includes("checklist")?"pop":n.includes("habilidade")||n.includes("capability")||n.includes("workflow")?"skill":n.includes("decidir")||n.includes("tradeoff")?"decision":"document"}function $n(a){return a.split(`
`).map(n=>n.match(/^#{1,4}\s+(.+)$/)?.[1]?.trim()).filter(Boolean).slice(0,12)}function zn(a){const n=a.split(/\n+/).map(o=>o.replace(/^[-*]\s+/,"").trim()).filter(o=>o.length>=48&&o.length<=220).filter(o=>/deve|precisa|permite|central|skill|contexto|agente|documento|conhecimento|gerar|unificar|analisar/i.test(o));return R(n).slice(0,8)}function Qe(a,n){return R(a.split(/\n+/).map(o=>o.replace(/^[-*]\s+/,"").trim()).filter(o=>o.length>20).filter(o=>n.some(s=>o.toLowerCase().includes(s)))).slice(0,6)}function qn(a,n){const o=a.toLowerCase(),s=[];return!o.includes("fonte")&&!o.includes("source")&&s.push("Fonte/origem pouco explícita."),o.includes("exemplo")||s.push("Faltam exemplos de uso."),n==="skill"&&!o.includes("entrada")&&s.push("Skill sem schema claro de entrada."),n==="skill"&&!o.includes("saída")&&s.push("Skill sem schema claro de saída."),(n==="pop"||n==="decision")&&!o.includes("status")&&s.push("Status editorial não definido."),s}function we(a){const n={};return a.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9\s-]/g," ").split(/\s+/).forEach(o=>{o.length>3&&!_n.has(o)&&(n[o]=(n[o]||0)+1)}),Object.entries(n).sort((o,s)=>s[1]-o[1]).map(([o])=>o)}function ya(a){const o=a.map(s=>s.recommendedOutput).reduce((s,r)=>(s[r]=(s[r]||0)+1,s),{});return Object.entries(o).sort((s,r)=>r[1]-s[1])[0]?.[0]||"document"}function Mn(a,n){const o=a.find(r=>r.title&&r.title.length>3)?.title,s={skill:"Skill consolidada",pop:"POP consolidado",prompt:"Prompt consolidado",decision:"Decisão consolidada",summary:"Resumo consolidado",document:"Documento consolidado",context_pack:"Pacote de contexto consolidado"};return o?`${s[n]}: ${o}`:s[n]}function Fn(a,n,o){const s=R(a.flatMap(r=>r.tags)).slice(0,5).join(", ");return`Hermione analisou ${o} fonte${o===1?"":"s"} e recomenda gerar um artefato do tipo ${n}. Temas principais: ${s||"conhecimento geral"}.`}function Kn(a){const n=R(a.analysis.flatMap(s=>s.keyIdeas)).slice(0,12),o=a.analysis.map((s,r)=>`- ${r+1}. **${s.name}**: ${s.detectedType}; ${s.keywords.slice(0,5).join(", ")}`).join(`
`);return`# ${a.title}
...
${a.tags.map(s=>`\`${s}\``).join(" ")}
`}function Gn(a,n){return n==="skill"?`# ${a.title}
