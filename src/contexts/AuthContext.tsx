  ...
  const signUp = async (
    ...
    if (data.user) {
      try {
        ...
        await notifyPendingApproval({ email, name, provider: "email" });
      } catch (approvalError) {
...
async function guardApproval(userId: string, email?: string): Promise<void> {
  ...
  if (status === "pending") {
    await supabase.auth.signOut();
    throw buildApprovalGuardError("pending", email);
  }

  if (status === "rejected") {
    await supabase.auth.signOut();
    throw buildApprovalGuardError("rejected", email);
  }
