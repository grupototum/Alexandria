  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const record = await fetchApprovalRecord(user.id);
        ...
        if (!record) {
          ...
          await notifyPendingApproval({ email: user.email ?? null, name, provider });
        ...
        let msg = err instanceof Error ? err.message : 'Erro ao processar login com Google.';
