...
const TEXT_FILE_PATTERN = /\.(md|markdown|txt|json)$/i;
...
export default function KnowledgeBridges() {
  ...
  const score = bridgePackage?.files.length
    ? Math.round((bridgePackage.importableFiles.length / bridgePackage.files.length) * 100)
    : 0;
  ...
  async function handleFiles(files: FileList | null) {
    ...
    const accepted = filterTextFiles(files);
    ...
    try {
      const contents = await readTextFiles(accepted);
      ...
      toast.error(error instanceof Error ? error.message : "Não foi possível ler os arquivos selecionados.");
    }
  }
  ...
  async function handleLogseqSkillFiles(files: FileList | null) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Não foi possível ler os arquivos do Logseq.");
    }
  }
...
function SkillSyncOverview({ preview }: { preview: KnowledgeSyncPreview }) {
  const healthTone =
    preview.health.status === "ready"
      ? "text-emerald-700"
      : preview.health.status === "blocked"
        ? "text-destructive"
        : "text-amber-700";
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-5">
        ...
        <DataPanel>
          <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-muted-foreground">Score sync</p>
          <p className={`mt-2 text-2xl font-semibold ${healthTone}`}>{preview.health.score}%</p>
        </DataPanel>
      </div>

      <Alert>
        <ShieldCheck className="h-4 w-4" />
        <AlertTitle>Status da rodada: {preview.health.status}</AlertTitle>
        <AlertDescription>
          <span className="block">{preview.health.nextAction}</span>
          {preview.health.warnings.length ? (
            <span className="mt-2 block text-xs">
              {preview.health.warnings.join(" ")}
            </span>
          ) : null}
        </AlertDescription>
      </Alert>
...
function filterTextFiles(files: FileList): File[] {
  return Array.from(files).filter((file) => TEXT_FILE_PATTERN.test(file.webkitRelativePath || file.name));
}
  ...
  try {
    ...
    throw new Error(
      error instanceof Error
        ? `Falha ao ler arquivo local: ${error.message}`
        : "Falha ao ler um dos arquivos locais."
    );
  }
}
