...
import { STEPS } from "./components/EditClientUtilities";
import { useEditClient } from "./hooks";
import type { FormData } from "./hooks";

/**
 * EditClient Container Component
 * Manages form state, validation, and step navigation
 */
export function EditClientLayout() {
  ...
  const { form, setForm, loading, saving, submit } = useEditClient(clientId);
    ...
    return (
      <AppLayout>
        <motion.div {...pageTransition} className="p-6 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <Button
            variant="outline"
            className="mt-4 border-border/40"
      ...
      <motion.div {...pageTransition} className="p-6 max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground uppercase tracking-widest mt-1">
            {form.company_name} · Etapa {step + 1} de 5
          </p>
        </motion.div>
