...
export const AgentChatLayout: React.FC = () => {
  ...
  const hadActiveStepsRef = useRef(false);
  ...
  const {
    ...
    enableRAG: true,
    onSuccess: (result) => {
      logger.log('Execução com sucesso:', result);
    },
    onError: (error) => {
  ...
  useEffect(() => {
    ...
    if (isExecuting) {
      hadActiveStepsRef.current = true;
    ...
    } else if (hadActiveStepsRef.current) {
      hadActiveStepsRef.current = false;
    ...
    try {
      ...
      if (result) {
        ...
        if (typeof res === 'string') {
          ...
          fullContent = stringifyOutput(res.text);
        } else if (res?.message) {
          fullContent = stringifyOutput(res.message);
        } else if (res?.output) {
          fullContent = Array.isArray(res.output)
            ? res.output
                .map(stringifyOutput)
                .join('\n\n')
            : stringifyOutput(res.output);
        } else if (res) {
