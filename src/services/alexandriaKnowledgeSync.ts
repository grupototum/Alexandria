...
export type KnowledgeSyncHealthStatus = "ready" | "needs_review" | "blocked" | "empty";

export interface KnowledgeSyncHealth {
  status: KnowledgeSyncHealthStatus;
  score: number;
  nextAction: string;
  warnings: string[];
}
...
export interface KnowledgeSyncPreview {
  ...
  duplicateFiles: KnowledgeSyncFileAnalysis[];
  health: KnowledgeSyncHealth;
}
...
export function buildKnowledgeSyncPreview(
  ...
  const analyses = markDuplicateFiles(files.map((file) => analyzeKnowledgeSyncFile(file)));
  const skillCandidates = analyses.filter((file) => file.isSkillCandidate);
  const duplicateFiles = analyses.filter((file) => file.isDuplicate);
  const importableSkills = skillCandidates.filter((file) => file.zone === "green" && !file.isDuplicate);
  const reviewFiles = skillCandidates.filter((file) => file.zone === "yellow" || file.isDuplicate);
  const blockedFiles = analyses.filter((file) => file.zone === "red");
  ...
  return {
    ...previewBase,
    health: summarizeKnowledgeSyncHealth(previewBase),
  };
}
...
export async function importKnowledgeSyncPreview(
  ...
  const artifact = await createArtifactFromSources(sourceInputs, {
    ...
    metadata: {
      ...
      duplicateFiles: preview.duplicateFiles.map((file) => ({
        name: file.name,
...
export function analyzeKnowledgeSyncFile(file: KnowledgeSyncFileInput): KnowledgeSyncFileAnalysis {
  ...
  return {
    ...file,
    identityKey: buildKnowledgeSyncIdentityKey(file),
    zone: privacy.zone,
...
export function summarizeKnowledgeSyncHealth(
  preview: Pick<
    KnowledgeSyncPreview,
    "files" | "skillCandidates" | "importableSkills" | "reviewFiles" | "blockedFiles" | "duplicateFiles"
  >
): KnowledgeSyncHealth {
  ...
  if (!preview.files.length) {
    return {
      status: "empty",
      score: 0,
      nextAction: "Selecionar uma pasta ou pacote de arquivos para análise.",
      warnings: ["Nenhum arquivo foi analisado nesta rodada."],
    };
  }

  if (!preview.skillCandidates.length) {
    warnings.push("Nenhum arquivo com formato de skill foi detectado.");
  }

  if (preview.duplicateFiles.length) {
    warnings.push(`${preview.duplicateFiles.length} arquivo(s) duplicado(s) ficaram fora da importação automática.`);
  }

  if (preview.blockedFiles.length) {
    warnings.push(`${preview.blockedFiles.length} arquivo(s) vermelho(s) bloqueado(s) por política de privacidade.`);
  }

  if (preview.reviewFiles.length) {
    warnings.push(`${preview.reviewFiles.length} skill(s) precisam de revisão antes de entrar na Alexandria.`);
  }

  const score = Math.round((preview.importableSkills.length / Math.max(preview.skillCandidates.length, 1)) * 100);

  if (preview.importableSkills.length) {
    return {
      status: preview.reviewFiles.length || preview.blockedFiles.length ? "needs_review" : "ready",
      score,
      nextAction: "Importar as skills verdes e manter duplicatas/amarelos para revisão.",
      warnings,
    };
  }

  return {
    status: preview.blockedFiles.length ? "blocked" : "needs_review",
    score,
    nextAction: "Revisar o pacote de origem antes de sincronizar.",
    warnings,
  };
}

export function buildKnowledgeSyncIdentityKey(file: KnowledgeSyncFileInput): string {
  return (file.externalId || file.sourcePath || file.name).trim().toLowerCase();
}

function markDuplicateFiles(files: KnowledgeSyncFileAnalysis[]): KnowledgeSyncFileAnalysis[] {
  const counts = files.reduce<Record<string, number>>((acc, file) => {
    acc[file.identityKey] = (acc[file.identityKey] || 0) + 1;
    return acc;
  }, {});
  ...
  return files.map((file) => {
    const duplicateCount = counts[file.identityKey] || 1;
    const firstSeen = seen.has(file.identityKey);
    seen.add(file.identityKey);

    return {
      ...file,
      duplicateCount,
      isDuplicate: duplicateCount > 1 && firstSeen,
    };
  });
}
