...
export async function createArtifactFromSources(
  ...
  const { error: versionError } = await (supabase as any).from("hermione_artifact_versions").insert({
    artifact_id: data.id,
    version: 1,
  ...
  if (versionError) {
    throw new Error(`Artefato criado, mas falhei ao registrar a versão: ${versionError.message}`);
  }

  if (options.sourceIds?.length) {
    const { error: sourceLinkError } = await (supabase as any).from("hermione_artifact_sources").insert(
    ...
    if (sourceLinkError) {
      throw new Error(`Artefato criado, mas falhei ao vincular fontes: ${sourceLinkError.message}`);
    }
  }
...
export async function logConsultation(input: {
  ...
  const { error } = await (supabase as any).from("hermione_consultations").insert({
    query: input.query,
  ...
  if (error) {
    throw new Error(`Não consegui registrar a consulta da Hermione: ${error.message}`);
  }
}
