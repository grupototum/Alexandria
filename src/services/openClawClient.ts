...
export async function executeAgent(
  ...
  if (OPENCLAW_CONFIG.MOCK_MODE || !isOpenClawBrowserReachable()) {
    logger.log('[OpenClaw] Mock mode - simulating execution', payload);
  ...
  try {
    ...
    if (OPENCLAW_CONFIG.ENABLE_LOGS) {
      logger.log('[OpenClaw] Execution result:', result);
    }
    ...
    if (retryCount < OPENCLAW_CONFIG.RETRY_ATTEMPTS) {
      logger.log(`[OpenClaw] Retrying... (${retryCount + 1}/${OPENCLAW_CONFIG.RETRY_ATTEMPTS})`);
